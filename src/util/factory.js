...
const d3 = require('d3')
const localCsv = require('./localcsv/localCsv')
const _ = {
...
const LocalCSVDocument = function (path) {
  var self = {}

  self.build = function () {
    const csvContents = localCsv.localCsv
    Promise.resolve(d3.csvParse(csvContents)).then(createBlips)
  }
  ...
  var createBlips = function (data) {
    try {
      ...
      plotRadar('Test tech radar', blips, 'CSV File', [])
    } catch (exception) {
...
const LocalCsvInput = function () {
  ...
  self.build = function () {
    sheet = LocalCSVDocument('../data.csv')
    sheet.init().build()
  }
